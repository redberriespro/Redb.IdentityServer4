...
namespace IdentityServer.IntegrationTests.Clients
{
    public class CustomTokenResponseClients
    {
        ...
        public async Task Extension_grant_success_should_return_custom_response()
        {
            ...
            // raw fields
            var fields = GetFields(response);
        ((JsonElement)fields["string_value"]).ToString().Should().Be("some_string");
        ((JsonElement)fields["int_value"]).GetInt32().Should().Be(42); 
        ...
        var responseObject = fields["dto"];
            ...
            var responseDto = GetDto((JsonElement) responseObject);
            var dto = CustomResponseDto.Create;
        ...
        payload["client_id"].Should().Be("client.custom");
        payload["sub"].Should().Be("bob");
        payload["idp"].Should().Be("local");
        payload["aud"].Should().Be("api");
         
        var scopes = ((JArray)payload["scope"]);
            scopes.First().ToString().Should().Be("api1");

        var amr = (JArray)payload["amr"];
            amr.Count().Should().Be(1);
